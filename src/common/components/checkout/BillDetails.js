...
const BillDetails = ({
  ...
  useEffect(() => {
    ...
    setDistricts(allDistricts.filter(district => district.parent_code === cityCode))
  }, [])
  ...
  const handleCityChange = (value) => {
    const districts = allDistricts.filter(district => district.parent_code === value.key)
  ...
  if (cartItems && cartItems.length >= 1) {
    return (
      <div className="billing-info-wrap">
        <Title level={2} style={{ textAlign: 'center' }}>Billing Details</Title>
        <Form
          size='middle'
          form={form}
        ...
        >
          ...
          <Title level={3} onClick={toggleAdditional} style={{ cursor: 'pointer', width: 'fit-content'}}>
            Additional information
            <span style={{ marginLeft: 10, verticalAlign: 'text-top'}}>
              { showAdditional ? <MinusCircleOutlined /> : <PlusCircleOutlined /> }
            </span>
          </Title>
          {
